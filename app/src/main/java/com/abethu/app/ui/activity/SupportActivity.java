...
import static com.abethu.app.network.APIConstants.APIs.SUPPORTENQURIY;
import static com.abethu.app.network.APIConstants.APIs.SUPPORTHELP;
...
import static com.abethu.app.network.APIConstants.Params.APP_VERSION;
import static com.abethu.app.network.APIConstants.Params.AUTHPROVIDER;
...
import static com.abethu.app.network.APIConstants.Params.USEREMAIL;
import static com.abethu.app.network.APIConstants.Params.USERNAME;
import static com.abethu.app.network.APIConstants.Params.USERPHONE;
...
import static com.abethu.app.network.APIConstants.Params.VERSION_CODE_APK;
import static com.abethu.app.ui.activity.MainActivity.CURRENT_IP;
import static com.abethu.app.ui.activity.MainActivity.supportArrayList;
...
//                sendQueryToBackend();
    ...
    private void setQueryTypeTag() {
        ...
        for (int i=0;i<supportArrayList.size();i++) {
            ...
            rowTextView.setTextSize(14f);
            if(obj.isChecked())
            {
                queryTypeID = obj.getId();
                querySting = obj.getName();
                rowTextView.setBackgroundResource(R.drawable.support_bg_enabled);
                rowTextView.setTextColor(ResourcesCompat.getColor(getResources(),R.color.colorPrimaryDark,null));
                rowTextView.setTypeface(null, Typeface.BOLD);
                rowTextView.setTextSize(16f);
            }else
            {
                rowTextView.setBackgroundResource(R.drawable.support_bg_disabled);
...
//        params.put(Params.PLAT, deviceDetails.get(PLAT));
    ...
    params.put(AUTHPROVIDER, "Abethu");


    Log.d("sendQuery", "sendQuery: 1"+token);//token
    Log.d("sendQuery", "sendQuery: 2"+id);
    Log.d("sendQuery", "sendQuery: 3"+querySting);//id
    Log.d("sendQuery", "sendQuery: 4"+id);
    Log.d("sendQuery", "sendQuery: 5"+deviceDetails.get(APPVERSION));
    Log.d("sendQuery", "sendQuery: 6"+deviceDetails.get(VERSION_CODE));
    ...
    call.enqueue(new Callback<String>() {
        ...
        public void onResponse(Call<String> call, Response<String> response) {
            ...
            try {
                ticketResponse = new JSONObject(response.body());
//                Log.d("test", "onResponse:00 "+ticketResponse);
                Toast.makeText(SupportActivity.this,getResources().getString(R.string.support_message),Toast.LENGTH_LONG).show();
